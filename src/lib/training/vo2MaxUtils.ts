...
export const analyzeVO2MaxTrend = (
  ...
  // Filter relative to the most recent rolling average rather than wall-clock "now".
  // This keeps historical datasets analyzable and avoids time-sensitive behavior in tests.
  const latestRollingDate = new Date(rollingAverages[rollingAverages.length - 1].date);
  const cutoffDate = new Date(latestRollingDate);
