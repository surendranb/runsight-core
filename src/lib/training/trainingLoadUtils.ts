 ...
 * Standard Banister TRIMP is:
 *   durationMinutes * HRr * 0.64 * exp(k * HRr)
 * where HRr is fractional heart-rate reserve.
 */
...
const calculateTRIMPWithHeartRate = (
  ...
  // Calculate fractional heart-rate reserve.
  const deltaHR = avgHR - restingHR;
  const hrReserve = maxHR - restingHR;
  const deltaHRr = deltaHR / hrReserve;

  // Banister formula constants (use a neutral middle-ground constant).
  const k = 1.8;

  // Calculate TRIMP using fractional reserve, not raw BPM delta.
  const trimp = duration * deltaHRr * 0.64 * Math.exp(k * deltaHRr);
...
const calculateEstimatedTRIMP = (
  ...
  // Map session-RPE style load onto a Banister-like TRIMP scale.
  const estimatedTRIMP = duration * estimatedRPE * 0.5;
