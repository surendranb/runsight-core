...
const createUserLikeRuns = (): EnrichedRun[] => {
  const source = [
    { daysAgo: 2, distance: 7073.8, movingTime: 3668, hr: 140 },
    { daysAgo: 3, distance: 8615.6, movingTime: 4418, hr: 140 },
    { daysAgo: 4, distance: 10022.2, movingTime: 5151, hr: 143 },
    { daysAgo: 6, distance: 7293.5, movingTime: 3751, hr: 139 },
    { daysAgo: 7, distance: 10024.9, movingTime: 5154, hr: 143 },
    { daysAgo: 8, distance: 7045.9, movingTime: 3690, hr: 143 },
    { daysAgo: 9, distance: 7067.2, movingTime: 3618, hr: 140 },
    { daysAgo: 10, distance: 8263.9, movingTime: 4155, hr: 141 },
    { daysAgo: 11, distance: 10029.9, movingTime: 4882, hr: 145 },
    { daysAgo: 12, distance: 8013.2, movingTime: 4095, hr: 146 },
    { daysAgo: 20, distance: 5018.6, movingTime: 2133, hr: 150 },
    { daysAgo: 21, distance: 10025.8, movingTime: 4442, hr: 145 },
    { daysAgo: 24, distance: 5265, movingTime: 2332, hr: 146 },
    { daysAgo: 27, distance: 10100.7, movingTime: 4548, hr: 145 },
  ];

  return source.map((run, index) => {
    const date = new Date();
    date.setDate(date.getDate() - run.daysAgo);
    return createMockRun({
      ...
      average_speed: run.distance / run.movingTime,
      average_heartrate: run.hr,
...
describe('Advanced Race Prediction Models', () => {
  ...
  describe('getMultipleRacePredictions', () => {
    ...
    it('should stay conservative when half-marathon evidence is limited', () => {
      const runs = createUserLikeRuns();
      const predictions = getMultipleRacePredictions(runs, undefined, {
        restingHeartRate: 60,
        maxHeartRate: 170
      });

      const tenK = predictions.find(prediction => prediction.distanceName === '10K');
      const halfMarathon = predictions.find(prediction => prediction.distanceName === 'Half Marathon');
      const marathon = predictions.find(prediction => prediction.distanceName === 'Marathon');

      expect(tenK).toBeDefined();
      expect(halfMarathon).toBeDefined();
      expect(marathon).toBeDefined();

      expect(halfMarathon!.predictedTime).toBeGreaterThan(2 * 3600);
      expect(halfMarathon!.predictedPace).toBeGreaterThan(tenK!.predictedPace);
      expect(marathon!.predictedPace).toBeGreaterThan(halfMarathon!.predictedPace);
      expect(halfMarathon!.confidence).toBeLessThanOrEqual(0.55);
      expect(marathon!.confidence).toBeLessThanOrEqual(0.45);
    });
  });
