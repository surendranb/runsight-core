...
const detectInadequateWarmup = (runs: EnrichedRun[]) => {
  ...
  // Estimate warmup adequacy based on faster efforts that likely need more preparation.
  // This keeps the heuristic deterministic instead of introducing random grades.
  const likelyUnderwarmedRuns = workoutRuns.filter(run => {
    const pace = run.moving_time / (run.distance / 1000);
    const highEffort = Boolean(run.average_heartrate && run.average_heartrate >= 160);
    const fastEffort = pace < 245;

    return highEffort && fastEffort;
  });

  const frequency = likelyUnderwarmedRuns.length / workoutRuns.length;

  if (frequency < 0.35) return null;

  const severity: 'minor' | 'moderate' | 'significant' =
    frequency > 0.7 ? 'significant' :
    frequency > 0.5 ? 'moderate' :
    'minor';
