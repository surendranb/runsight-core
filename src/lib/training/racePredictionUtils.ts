...
interface PredictionEvidence {
  ...
  sourcePace: number;
  daysAgo: number;
  type: 'distance-specific' | 'near-distance' | 'benchmark';
  weight: number;
}
...
export const calculateAdvancedRacePrediction = (
  ...
  if (recentRuns.length < 5) {
    return createMinimalPrediction(raceDistance, distanceName, recentRuns);
  }

  // Calculate current fitness metrics
  const currentVO2Max = calculateCurrentVO2Max(recentRuns, userPhysiology);
  const fitnessProgression = analyzeFitnessProgression(recentRuns);
  const trainingLoadAnalysis = analyzeTrainingLoad(recentRuns);
  const evidence = buildPredictionEvidence(recentRuns, raceDistance);

  if (evidence.length === 0) {
    return createMinimalPrediction(raceDistance, distanceName, recentRuns);
  }

  // Base prediction on recent performance evidence, not optimistic physiology heuristics.
  let predictedTime = calculateEvidenceAnchoredPrediction(
    evidence,
    raceDistance,
  ...
  // Use physiology as a conservative floor when available.
  if (currentVO2Max) {
    const vo2Prediction = calculateVO2MaxBasedPrediction(raceDistance, currentVO2Max);
    const targetKm = raceDistance / 1000;
    const floorFactor = targetKm >= 21.097 ? 0.97 : 0.94;
    predictedTime = Math.max(predictedTime, vo2Prediction * floorFactor);
  }
  ...
  predictedTime = Math.max(raceDistance * 0.18, predictedTime + environmentalAdjustment);

  const confidence = calculateEvidenceBasedConfidence(
    evidence,
    recentRuns,
    currentVO2Max,
    trainingLoadAnalysis,
  ...
  const confidenceInterval = calculateEvidenceBasedConfidenceInterval(
    predictedTime,
    confidence,
    evidence,
    raceDistance
  ...
  const recommendations = generateRaceRecommendations(
    ...
    raceDistance,
    environmentalAdjustments,
    buildEvidenceRecommendations(evidence, raceDistance)
  );
...
const calculateCurrentVO2Max = (
  ...
  const vo2MaxValues = vo2MaxRuns
    .map(run => calculateVO2Max(run, {
      restingHeartRate: userPhysiology.restingHeartRate || 60,
      maxHeartRate: userPhysiology.maxHeartRate || 190
    }))
...
const buildPredictionEvidence = (
  ...
  return recentRuns
    .filter(run => run.distance >= 3000 && run.moving_time > 0)
    .map((run) => {
      const sourceDistanceKm = run.distance / 1000;
      const sourcePace = run.moving_time / sourceDistanceKm;
      const coverage = sourceDistanceKm / targetKm;

      if (coverage < Math.min(0.4, targetKm <= 10 ? 0.6 : 0.45)) {
        return null;
      }

      const distanceDelta = Math.abs(sourceDistanceKm - targetKm) / targetKm;
      const type: PredictionEvidence['type'] =
        distanceDelta <= 0.15
          ? 'distance-specific'
          : coverage >= 0.75
            ? 'near-distance'
      ...
      const predictedTime = run.moving_time * Math.pow(
        targetKm / sourceDistanceKm,
        getRiegelExponent(sourceDistanceKm, targetKm)
      );

      const daysAgo = Math.max(
        0,
        Math.round((Date.now() - new Date(run.start_date).getTime()) / (1000 * 60 * 60 * 24))
      );

      const typeWeight =
        type === 'distance-specific' ? 1.3 :
        type === 'near-distance' ? 1.0 :
        0.7;
      const recencyWeight = Math.max(0.4, 1 - (daysAgo / 150));
      ...
      return {
        ...
        daysAgo,
        type,
        weight: typeWeight * recencyWeight
      };
    })
    .filter((evidence): evidence is PredictionEvidence => Boolean(evidence));
};

const getRiegelExponent = (sourceDistanceKm: number, targetDistanceKm: number): number => {
  let exponent = 1.06;

  if (targetDistanceKm >= 21.097 && sourceDistanceKm < targetDistanceKm * 0.75) {
    exponent += 0.03;
  }

  if (targetDistanceKm >= 42.195 && sourceDistanceKm < targetDistanceKm * 0.6) {
    exponent += 0.03;
  }

  if (sourceDistanceKm < 5 && targetDistanceKm >= 21.097) {
    exponent += 0.02;
  }
...
const calculateEvidenceAnchoredPrediction = (
  evidence: PredictionEvidence[],
  raceDistance: number,
  trainingLoad: ReturnType<typeof analyzeTrainingLoad>,
  fitnessProgression: ReturnType<typeof analyzeFitnessProgression>
): number => {
  const weightedMedian = calculateWeightedMedianPrediction(evidence);
  const targetKm = raceDistance / 1000;
  const maxCoverage = Math.max(...evidence.map(item => item.sourceDistanceKm / targetKm));
  const hasDistanceSpecificEvidence = evidence.some(item => item.type === 'distance-specific');

  let adjustedTime = weightedMedian;

  if (!hasDistanceSpecificEvidence) {
    adjustedTime *= 1.02;
  }

  if (targetKm >= 21.097 && maxCoverage < 0.75) {
    adjustedTime *= 1.05;
  }

  if (targetKm >= 42.195 && maxCoverage < 0.75) {
    adjustedTime *= 1.08;
  }
  ...
  const conservativePaceFloor = getConservativePaceFloor(evidence, raceDistance);
  const adjustedPace = Math.max(adjustedTime / targetKm, conservativePaceFloor);

  return adjustedPace * targetKm;
};

const calculateWeightedMedianPrediction = (evidence: PredictionEvidence[]): number => {
  const sorted = [...evidence].sort((a, b) => a.predictedTime - b.predictedTime);
  ...
  return sorted[sorted.length - 1]?.predictedTime || 0;
};

const getConservativePaceFloor = (
  evidence: PredictionEvidence[],
  raceDistance: number
): number => {
  const targetKm = raceDistance / 1000;

  const bestNear10kPace = evidence
    .filter(item => item.sourceDistanceKm >= 8)
    .reduce<number | undefined>((best, item) => (
      best === undefined ? item.sourcePace : Math.min(best, item.sourcePace)
    ), undefined);

  const bestNear5kPace = evidence
    .filter(item => item.sourceDistanceKm >= 4.5)
    .reduce<number | undefined>((best, item) => (
      best === undefined ? item.sourcePace : Math.min(best, item.sourcePace)
    ), undefined);
  ...
  if (targetKm > 10 && bestNear10kPace !== undefined) {
    floor = Math.max(floor, bestNear10kPace + (targetKm <= 21.097 ? 20 : 35));
  }

  if (targetKm > 5 && bestNear5kPace !== undefined) {
    floor = Math.max(floor, bestNear5kPace + (targetKm <= 10 ? 10 : 20));
  }
...
const generateRaceRecommendations = (
  ...
  fitnessProgression: ReturnType<typeof analyzeFitnessProgression>,
  raceDistance: number,
  environmentalAdjustments: RacePrediction['environmentalAdjustments'] | undefined,
...
const buildEvidenceRecommendations = (
  evidence: PredictionEvidence[],
  raceDistance: number
): string[] => {
  const targetKm = raceDistance / 1000;
  ...
  if (!hasDistanceSpecificEvidence) {
    notes.push('Prediction is anchored to shorter recent runs, so it is intentionally conservative.');
  }

  if (targetKm >= 21.097 && maxCoverage < 0.75) {
    notes.push('Recent long-run evidence is limited for this distance; treat this prediction as a planning estimate, not race-ready proof.');
  }

  if (targetKm >= 42.195 && maxCoverage < 0.75) {
    notes.push('Marathon prediction confidence is capped until you build longer long-run evidence.');
  }

  return notes;
};

const calculateEvidenceBasedConfidence = (
  evidence: PredictionEvidence[],
  recentRuns: EnrichedRun[],
  currentVO2Max: number | undefined,
  trainingLoad: ReturnType<typeof analyzeTrainingLoad>,
  raceDistance: number
): number => {
  const targetKm = raceDistance / 1000;
  const maxCoverage = Math.max(...evidence.map(item => item.sourceDistanceKm / targetKm));
  const distanceSpecificCount = evidence.filter(item => item.type === 'distance-specific').length;
  const predictedTimes = evidence.map(item => item.predictedTime);
  const mean = predictedTimes.reduce((sum, value) => sum + value, 0) / predictedTimes.length;
  const variance = predictedTimes.reduce((sum, value) => sum + Math.pow(value - mean, 2), 0) / predictedTimes.length;
  const coefficientOfVariation = mean > 0 ? Math.sqrt(variance) / mean : 0;

  let confidence = 0.3;

  if (recentRuns.length >= 20) confidence += 0.15;
  else if (recentRuns.length >= 10) confidence += 0.1;

  if (distanceSpecificCount > 0) confidence += 0.2;
  else if (maxCoverage >= 0.75) confidence += 0.12;
  else if (maxCoverage >= 0.5) confidence += 0.06;

  if (evidence.length >= 5) confidence += 0.1;
  else if (evidence.length >= 3) confidence += 0.05;

  if (currentVO2Max) confidence += 0.05;
  ...
  if (coefficientOfVariation > 0.12) confidence -= 0.1;
  else if (coefficientOfVariation > 0.08) confidence -= 0.05;
...
const calculateEvidenceBasedConfidenceInterval = (
  predictedTime: number,
  confidence: number,
  evidence: PredictionEvidence[],
  raceDistance: number
  ...
  const targetKm = raceDistance / 1000;
  const maxCoverage = Math.max(...evidence.map(item => item.sourceDistanceKm / targetKm));
  const sortedTimes = evidence.map(item => item.predictedTime).sort((a, b) => a - b);
  const spread = sortedTimes.length > 1
    ? (sortedTimes[sortedTimes.length - 1] - sortedTimes[0]) / Math.max(predictedTime, 1)
    : 0;

  let variancePercent = 0.06 + ((1 - confidence) * 0.14);
  variancePercent = Math.max(variancePercent, spread * 0.5);

  if (targetKm >= 21.097 && maxCoverage < 0.75) {
    variancePercent += 0.03;
  }

  const variance = predictedTime * Math.min(0.22, variancePercent);
...
const createMinimalPrediction = (
  distance: number,
  distanceName: string,
  recentRuns: EnrichedRun[] = []
): RacePrediction => {
  const fallbackPace = recentRuns.length > 0
    ? recentRuns.reduce((sum, run) => sum + (run.moving_time / (run.distance / 1000)), 0) / recentRuns.length
    : 420; // 7:00/km default
  const estimatedPace = Math.max(360, Math.round(fallbackPace * 1.05));
...
export const getMultipleRacePredictions = (
  ...
  const predictions = distances.map(distance =>
    calculateAdvancedRacePrediction(runs, distance, raceConditions, userPhysiology)
  );

  if (raceConditions) {
    predictions.forEach((prediction) => {
      if (!prediction.environmentalAdjustments) {
        const envResult = calculateEnvironmentalRaceAdjustment(raceConditions, prediction.distance);
        prediction.environmentalAdjustments = {
          temperature: raceConditions.temperature || 20,
          humidity: raceConditions.humidity || 60,
          elevation: raceConditions.elevationGain || 0,
  ...
  for (let index = 1; index < predictions.length; index++) {
    const previous = predictions[index - 1];
    const current = predictions[index];
    const minimumSlowdown =
      current.distance <= 10000 ? 10 :
      current.distance <= 21097 ? 20 :
      30;

    if (current.predictedPace < previous.predictedPace + minimumSlowdown) {
      current.predictedPace = previous.predictedPace + minimumSlowdown;
      current.predictedTime = Math.round(current.predictedPace * (current.distance / 1000));
      current.confidence = Math.min(current.confidence, current.distance >= 21097 ? 0.55 : 0.7);
      current.confidenceInterval = calculateEvidenceBasedConfidenceInterval(
        current.predictedTime,
        current.confidence,
        [{ predictedTime: current.predictedTime, sourceDistanceKm: previous.distance / 1000, sourcePace: previous.predictedPace, daysAgo: 0, type: 'benchmark', weight: 1 }],
        current.distance
      );
    }
  }
