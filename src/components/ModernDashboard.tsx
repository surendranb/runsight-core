...
// import { EmptyDashboard } from './dashboard/EmptyDashboard'; // REMOVED: EmptyDashboard is no longer conditionally rendered here
...
interface ModernDashboardProps {
  ...
  isLoading: boolean;
  error: string | null;
  onSync?: (period: TimePeriod) => void; // Modified to accept period
  onLogout: () => void;
}
...
export const ModernDashboard: React.FC<ModernDashboardProps> = ({
  ...
  const toggleSection = (section: string) => {
    ...
    recordInteraction('sectionToggle', { section, expanded: newExpanded });
  };
  ...
  const filteredRuns = useMemo(() => {
    ...
    switch (selectedPeriod) {
      ...
      case 'lastMonth':
        ...
        dateFilteredRuns = runs.filter(run => new Date(run.start_date_local) >= startDate && new Date(run.start_date_local) <= endDate);
        break;
      case 'thisYear':
      ...
      case 'lastYear':
        ...
        dateFilteredRuns = runs.filter(run => new Date(run.start_date_local) >= startDate && new Date(run.start_date_local) <= lastYearEnd);
        break;
      case 'allTime':
      default:
    ...
    if (outlierStats.outliers > 0) {
      console.log(`Filtered out ${outlierStats.outliers} outlier runs out of ${outlierStats.total} total runs:`, outlierStats.outlierReasons);
    }
    return cleanRuns;
  }, [runs, selectedPeriod]);
  ...
  const metrics = useMemo(() => {
    if (filteredRuns.length === 0) {
      return { totalRuns: 0, totalDistance: 0, totalTime: 0, avgPace: 0, avgPacePrevious: 0, paceImprovement: 0 };
    }
    ...
    const periodDays = selectedPeriod === 'last7' ? 7 : selectedPeriod === 'last30' ? 30 : selectedPeriod === 'last90' ? 90 : selectedPeriod === 'thisYear' ? 365 : 90;
    ...
    return { totalRuns: filteredRuns.length, totalDistance, totalTime, avgPace, avgPacePrevious, paceImprovement };
  }, [filteredRuns, runs, selectedPeriod]);

  const significantChanges = useMemo(() => getHighlightedPatterns(filteredRuns), [filteredRuns]);
  ...
  const getDataFreshnessIndicator = () => {
    ...
    const runDateStr = latestRun.start_date_local.substring(0, 10);
  ...
  if (error) {
    ...
    const errorWithRecovery = {
      ...translatedError,
      recoveryOptions: [
        { label: 'Retry Loading', description: 'Try loading the dashboard again', action: () => window.location.reload(), primary: true },
        { label: 'Sync Data', description: 'Sync your running data from Strava', action: () => onSync?.('30days') }
      ]
    };
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 p-4">
        <div className="max-w-md w-full">
          <div className="text-center mb-6">
            ...
            <h1 className="text-2xl font-bold text-gray-900 mb-2">RunSight Dashboard</h1>
          </div>
          <ErrorDisplay error={errorWithRecovery} className="shadow-lg" />
        </div>
      </div>
    );
  }

  // Check if runs data is empty and display a message (or the new EmptyDashboard component)
  if (runs.length === 0) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
        <div className="text-center p-8 bg-white shadow-lg rounded-lg max-w-2xl mx-auto">
          <div className="text-5xl mb-4 text-gray-400">📊</div>
          <h2 className="text-3xl font-bold text-gray-800 mb-2">No Runs Data</h2>
          <p className="text-gray-600 text-lg mb-6">
            Your dashboard is awaiting your Strava activities.
          </p>
          {onSync && (
             <StandardButton 
               onClick={() => onSync('allTime')} 
           ...
           <p className="text-gray-500 text-sm mt-4">
            If you expected to see data, try syncing or reloading the page.
           </p>
        </div>
      </div>
    );
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div className="space-y-8">
          ...
          <Section
            ...
            level={2}
            icon={TrendingUpIcon}
            badge={{ text: `${filteredRuns.length} runs analyzed`, color: 'blue' }}
            actions={<span className="text-xs text-gray-500">{getDataFreshnessIndicator()}</span>}
          >
            <PaceTrendChart
              data={filteredRuns}
              period={getPeriodLabel()}
          ...
          <Section
            ...
            level={2}
            icon={Clock}
            badge={{ text: `${filteredRuns.length} runs from ${getPeriodLabel().toLowerCase()}`, color: 'green' }}
            actions={<span className="text-xs text-gray-500">Most recent first</span>}
          >
            <ActivityTimeline
