...
interface NavigationBarProps {
  ...
  userName?: string;
  onLogout?: () => void;
  onSyncData?: (period: SyncPeriod, customRange?: { start: string, end: string }) => void; // Modified to pass SyncPeriod and optional range
  isSyncing?: boolean;
}
...
export const NavigationBar: React.FC<NavigationBarProps> = ({
  ...
  const handleSyncClick = () => {
    if (onSyncData) {
      if (selectedSyncPeriod === 'custom') {
        if (!customRange.start || !customRange.end) {
          alert('Please select both start and end dates');
          return;
        }
        onSyncData(selectedSyncPeriod, customRange);
      } else {
