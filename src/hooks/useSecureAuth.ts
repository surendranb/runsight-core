...
export const useSecureAuth = () => {
  ...
  const checkExistingSession = async () => {
    try {
      const user = await apiClient.getCurrentSessionUser();
  ...
  const handleStravaCallback = useCallback(async (code: string, state: string) => {
    try {
      setAuthState(prev => ({ ...prev, isLoading: true, error: null }));
      
      // Exchange code for user session (server-side)
      const { user } = await apiClient.authenticateWithStrava(code, state);
